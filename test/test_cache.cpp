...
TEST_F(CacheTest, WriteThrough_LRU)
{
    ...
    ASSERT_EQ(cache.get_statistics().cycles, ((MEM_DELAY * 16) + 2) + 7);
    // Check load miss
    int prev_cycles = cache.get_statistics().cycles;
    for (int i = 1; i < 8; i++)
    {
        cache.proc_request({Load, i * 8 * 8});
    }
    ...
    // check save miss
    cache.proc_request({Save, 0});
    ASSERT_EQ(cache.get_statistics().cycles, 2 + 2 * MEM_DELAY * 16);
    prev_cycles = cache.get_statistics().cycles;
    for (int i = 1; i < 8; i++)
    {
        cache.proc_request({Save, i});
    }
    ASSERT_EQ(cache.get_statistics().cycles, 1 * 7 + prev_cycles);
    prev_cycles = cache.get_statistics().cycles;
    for (int i = 1; i < 8; i++)
    {
        cache.proc_request({Save, i * 64 * 64});
    };
    ASSERT_EQ(cache.get_statistics().cycles, 7 * (2 * MEM_DELAY * 16 + 2) + prev_cycles);
    prev_cycles = cache.get_statistics().cycles;
    // Check set full
    cache.proc_request({Save, 9 * 64 * 64});
    ASSERT_EQ(cache.get_statistics().cycles, prev_cycles + 2 * MEM_DELAY * 16 + 2);
    ...
    prev_cycles = cache.get_statistics().cycles;
    cache.proc_request({Save, 64 * 64 * 9});
    ASSERT_EQ(cache.get_statistics().load_hits, 0);
    ASSERT_EQ(cache.get_statistics().store_hits, 0);
}
...
TEST_F(CacheTest, WriteBack_NoAllocate_LRU)
{
    ...
    cache = Cache{params};
    // check save hit -> no hits actually all should miss
    cache.proc_request({Save, 0});
    ASSERT_EQ(cache.get_statistics().cycles, 1 + MEM_DELAY * 16);
    prev_cycles = cache.get_statistics().cycles;
    for (int i = 1; i < 8; i++)
    {
        cache.proc_request({Save, i});
    }
    ASSERT_EQ(cache.get_statistics().cycles, 7 * (1 + MEM_DELAY * 16) + prev_cycles);
    prev_cycles = cache.get_statistics().cycles;
    // TODO check dirty bit with load
    // All of these should be non dirty misses
    for (int i = 1; i < 8; i++)
    {
        cache.proc_request({Save, i * 64 * 64});
    };
    ASSERT_EQ(cache.get_statistics().cycles, 7 * (MEM_DELAY * 16 + 1) + prev_cycles);
    prev_cycles = cache.get_statistics().cycles;
    // Check set full and dirty bit
    cache.proc_request({Save, 9 * 64 * 64});
    ASSERT_EQ(cache.get_statistics().cycles, prev_cycles + 16 * MEM_DELAY + 1);
    ...
    prev_cycles = cache.get_statistics().cycles;
    cache.proc_request({Save, 64 * 64 * 9});
    ASSERT_EQ(cache.get_statistics().cycles, prev_cycles + 1 + (MEM_DELAY * 16));
...
TEST_F(CacheTest, WriteThrough_NoAllocate_LRU)
{
    ...
    // Check set full
    cache.proc_request({Save, 9 * 64 * 64});
    ASSERT_EQ(cache.get_statistics().cycles, prev_cycles + 1 * MEM_DELAY * 16 + 1);
    ...
    cache.proc_request({Save, 64 * 64 * 9});
    ASSERT_EQ(cache.get_statistics().store_hits, 0);
    cache.proc_request({Save, 64 * 64 * 0});
    ASSERT_EQ(cache.get_statistics().store_hits, 1);
    ASSERT_EQ(cache.get_statistics().load_hits, 0);
}
